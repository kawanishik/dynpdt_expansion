#ifndef POPLAR_TRIE_MAP_SD_HPP
#define POPLAR_TRIE_MAP_SD_HPP
...
class map_SD {
  ...
  public:
    using this_type = map_SD<Trie, NLM>;
  ...
  public:
    ...
    // 2**capa_bits.
    explicit map_SD(uint32_t capa_bits, uint64_t lambda = 32) {
    ...
    const value_type* find(char_range key) const {
        ...
        while (!key.empty()) {
            ...
            // }
            if(lambda_ <= match) {
                node_id = hash_trie_.find_skip_dummy(node_id, match/lambda_);
                if(node_id == nil_id) return nullptr;
                match -= lambda_ * (match / lambda_);
            }
    ...
    value_type* update(char_range key) {
        ...
        while (!key.empty()) {
            ...
            if(pre_node_id != node_id) {   // ダミーノードで移動した場合
                hash_trie_.add_new_skip_dummy(pre_node_id, node_id, pre_match/lambda_);
            }
    ...
    void show_stats(std::ostream& os, int n = 0) const {
        ...
        show_stat(os, indent, "name", "map_SD");
    ...
    map_SD(const map_SD&) = delete;
    map_SD& operator=(const map_SD&) = delete;

    map_SD(map_SD&&) noexcept = default;
    map_SD& operator=(map_SD&&) noexcept = default;
